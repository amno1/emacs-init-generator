...
The tool can now bake `quicstart' file (package-quickstart.el) into init
...
If this option is on the tool will remove /package-quicstart.el/ (and .elc if
exists) from the .emacs.d directory.

If this option is nil, quickstart will be on, and tool will generate new
...
- does a lot of assumption; not very general, nor flexible, nor robust
