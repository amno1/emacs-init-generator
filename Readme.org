...
This is a defered setup, so most generally we wish to run code in either eval-after-load
or in some mode hook.
...
/genereate-init-files/ - generates early-init.el and init.el fromt the provided
                       configuration.
...
A tip: if you put generator.org in your .emacs.d directory, you don't need to run
/install-init-files/ command, since the tool generates init files in same directory
where the tool is. 

Another tip: while experimenting and writing a configuration, it is possilbe to make a misstake
and end-up with a non-working init file. For this reason I will always test the configuration by
running another instance of Emacs, with M-& emacs or from the command line. If everything starts
without problems I will then (maybe) restart my Emacs.

If you end-up with error in your init file, and don't have a running Emacs process, then
either run Emacs with -Q option or --debug-init and open the tool, edit the misstake and generate
new init file(s).
